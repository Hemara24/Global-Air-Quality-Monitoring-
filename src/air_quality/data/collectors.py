...
class OpenWeatherMapCollector:
    ...
    def _parse_response(self, data: Dict[Any, Any], location: Location) -> AirQualityReading:
...
class SimulatedDataCollector:
    ...
    def fetch_current_data(self, location: Location) -> AirQualityReading:
        ...
        base_data = self.locations_data.get(location.name, {"base_aqi": 75, "variation": 20})
        ...
        pm25_concentration = max(0, random.normalvariate(base_aqi * 0.4, variation * 0.2))
        readings.append(PollutantReading(
...
class DataCollectorFactory:
    ...
    def create_collector(collector_type: str, **kwargs):
        ...
        if collector_type.lower() == "openweathermap":
            ...
            if not api_key:
                raise ValueError("OpenWeatherMap collector requires 'api_key' parameter")
